...
class AttNet(nn.Module):
    ...
    def stage_forward(self, point_feat, pcds_coord, pcds_sphere_coord, query_embed_store=None, use_query_store=False, return_query=False):
        ...
        return pred_cls, pred_res_cls_0, pred_res_cls_1, pred_res_cls_2, query_embed_store
    ...
    def single_forward(self, batch, query_embed_store=None, use_query_store=False, return_query=False):
        ...
        pred_cls, pred_res_cls_0, pred_res_cls_1, pred_res_cls_2, query_embed_store = self.stage_forward(batch['pcds_xyzi'], batch['pcds_coord'], batch['pcds_sphere_coord'],
                                                                                                         query_embed_store, use_query_store, return_query)

        bs, time_num, _, _ = pred_cls.shape
        pred_res_cls_0 = pred_res_cls_0.view(bs, time_num, -1).unsqueeze(-1)
        pred_res_cls_1 = pred_res_cls_1.view(bs, time_num, -1).unsqueeze(-1)
        pred_res_cls_2 = pred_res_cls_2.view(bs, time_num, -1).unsqueeze(-1)
        ...
        loss2 = self.criterion_seg_cate(pred_res_cls_0, pcds_bev_target) + 3 * lovasz_softmax(pred_res_cls_0, pcds_bev_target, ignore=0)
        loss3 = self.criterion_seg_cate(pred_res_cls_1, pcds_bev_target) + 3 * lovasz_softmax(pred_res_cls_1, pcds_bev_target, ignore=0)
        loss4 = self.criterion_seg_cate(pred_res_cls_2, pcds_bev_target) + 3 * lovasz_softmax(pred_res_cls_2, pcds_bev_target, ignore=0)
    ...
    def infer(self, batch, i, query_embed_store=None):
        ...
        '''
        if i == 0:
            pred_cls, pred_res_cls_0, pred_res_cls_1, pred_res_cls_2, query_embed_store = self.stage_forward(batch['pcds_xyzi'].squeeze(0),
