...
def val(epoch, model, val_loader, category_list, save_path, tb_logger, learning_map_inv, rank=0):
    ...
    val_bf_save_path = os.path.join(save_path, 'val_bf_results/sequences')
    with torch.no_grad():
        for i, batch_dict in tqdm.tqdm(enumerate(val_loader)):
            ...
            #   visualization.draw_single_lidar_with_label(pcds_xyzi, bf_pred_cls)
            ...
            item_test_bf_save_path = os.path.join(val_bf_save_path, batch_dict['seq_id'][0], 'predictions')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tag', help='config file path', type=str, default='base_seg')
    parser.add_argument('--local_rank', type=int, default=0)
    parser.add_argument('--start_val_epoch', type=int, default=10)
    parser.add_argument('--end_val_epoch', type=int, default=11)
