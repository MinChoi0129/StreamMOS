...
def main(args, config):
    ...
    # load pretrain model
    pretrain_model = os.path.join(args.checkpoint_path)
    if os.path.exists(pretrain_model):
        base_net.load_state_dict(torch.load(pretrain_model, map_location='cpu'), strict=False)
        logger.info("Load model from {}".format(pretrain_model))
    freeze = True
    if freeze == True:
        for param in base_net.parameters():
            param.requires_grad = False
        for param in base_net.refine.parameters():
