...
class Livox_MultiClassMetric:
    ...
    def addBatch(self, gt, pred):
        # gt (bs, )
        # pred (bs, c); c denotes number of categories
        gt = gt.cpu() + 1  # static: 1, motion: 2 (consistent with KITTI)
        pred = pred.cpu()
        ...
        gt_map = (gt.float()).data.cpu().numpy()
        pred_map = (pred_map.float()).data.cpu().numpy() + 1  # static: 1, motion: 2 (consistent with KITTI)
