...
class PositionEmbeddingLearned(nn.Module):
    ...
    def forward(self, tensor):
        h, w = tensor.shape[-2:]
        i = torch.arange(w, device=tensor.device)  # 0~w
        j = torch.arange(h, device=tensor.device)  # 0~h
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        # ========== encoder ========== #
        encoder_layer = TransformerEncoderLayer(d_model,  # d_model -> hidden_dim
                                                nhead,  # head number
                                                dim_feedforward,
                                                dropout,  # dropout
                                                activation,  # relu
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.activation = _get_activation_fn(activation)  # rulu
    ...
    def forward_post(self,
                     ...
                     src_key_padding_mask: Optional[Tensor] = None,  # [bs, s]
                     pos: Optional[Tensor] = None):
        # ------- Multi-Head Self-Attention -------
        q = k = self.with_pos_embed(src, pos)
        src2 = self.self_attn(q, k, value=src, attn_mask=src_mask,  # [s, bs, c]
                              key_padding_mask=src_key_padding_mask)[0]
        src = src + self.dropout1(src2)  # dropout
        src = self.norm1(src)  # norm

        # ------- FFN -------
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))  # [s, bs, c]
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt = tgt + self.dropout3(tgt2)  # add && norm
        tgt = self.norm3(tgt)
